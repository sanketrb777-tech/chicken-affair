...
export default function KDSPage() {
  const [kots, setKots]               = useState([])         // all active KOTs
  const [categories, setCategories]   = useState([])         // sorted by sort_order
  const [kotItems, setKotItems]       = useState({})         // kotItemId -> is_done
  const [collapsedCats, setCollapsedCats] = useState({})     // catId -> bool
  ...
  async function fetchAll() {
    // Fetch categories sorted by priority
    ...
    // Fetch active KOTs with all nested data
    const { data, error } = await supabase
      .from('kots')
      .select(`
        id, status, created_at,
        orders ( id, order_type, customer_name, covers, cafe_tables ( number, name ) ),
        kot_items (
          id, is_done,
          order_items (
            id, quantity, notes,
    ...
    // Build kotItems map from DB state
    const itemMap = {}
    kotList.forEach(kot => {
      kot.kot_items.forEach(ki => {
        itemMap[ki.id] = ki.is_done
      })
    })
    setKotItems(prev => {
      // Merge: DB is source of truth, but keep any optimistic updates
      return { ...itemMap, ...Object.fromEntries(Object.entries(prev).filter(([k]) => itemMap[k] !== undefined)) }
    })
  }

  // Toggle a single kot_item
  async function toggleItem(kotItemId, currentDone) {
    ...
    // Check if all items in this KOT are now done → mark KOT ready
    const kot = kots.find(k => k.kot_items.some(ki => ki.id === kotItemId))
    if (kot && newDone) {
  ...
  // Mark entire KOT as Food Ready
  async function markFoodReady(kot) {
    // Mark all items done
    ...
    setKotItems(prev => {
      const next = { ...prev }
      kot.kot_items.forEach(ki => { next[ki.id] = true })
      return next
    })
  }
  ...
  function getOrderLabel(kot) {
    const type = kot.orders?.order_type
    const tableNum = kot.orders?.cafe_tables?.number
    const name = kot.orders?.customer_name
    if (type === 'dine_in')  return { label: 'Dine In', sub: `Table ${tableNum}` }
    if (type === 'takeaway') return { label: 'Takeaway', sub: name || '' }
    if (type === 'delivery') return { label: 'Delivery', sub: name || '' }
    return { label: 'Order', sub: '' }
  }
  ...
  // Aggregate all PENDING items across all KOTs, grouped by category
  const batchByCat = {}
  kots.forEach(kot => {
    kot.kot_items.forEach(ki => {
      const isDone = kotItems[ki.id] ?? ki.is_done
      if (isDone) return // skip done items
  ...
  // Sort categories by sort_order, skip empty ones
  ...
  const pendingKots = kots.filter(k => k.status !== 'ready')
            ...
            batchCategories.map(cat => (
              <div key={cat.id} style={{ borderBottom: '1px solid ' + BORDER }}>
                ...
                <div onClick={() => toggleCat(cat.id)}
                  ...
                  <span style={{ fontSize: 14, color: TEXT_LIGHT, transform: collapsedCats[cat.id] ? 'rotate(-90deg)' : 'rotate(0)', transition: 'transform 0.15s' }}>∨</span>
                </div>
                {/* Items */}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          {pendingKots.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
              {pendingKots.map((kot, kotIdx) => {
                const { label, sub } = getOrderLabel(kot)
                const elapsed         = getElapsed(kot.created_at)
                const elapsedColor    = getElapsedColor(kot.created_at)
                const allItemsDone    = kot.kot_items.every(ki => kotItems[ki.id] ?? ki.is_done)
                const tokenNo         = kotIdx + 1

                return (
                  <div key={kot.id} style={{ background: WHITE, borderRadius: 12, border: '1px solid ' + BORDER, overflow: 'hidden', boxShadow: '0 1px 6px rgba(0,0,0,0.07)' }}>

                    {/* Card header row 1 — order type + table number */}
                    <div style={{ background: allItemsDone ? TEAL_BG : '#F9FAFB', padding: '10px 14px', borderBottom: '1px solid ' + BORDER, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontWeight: 900, fontSize: 16, color: TEXT_DARK }}>{kot.orders?.cafe_tables?.number ? `T${kot.orders.cafe_tables.number}` : '#' + tokenNo}</span>
                    </div>

                    {/* Card header row 2 — token, time, biller */}
                    <div style={{ padding: '7px 14px', borderBottom: '1px solid ' + BORDER, display: 'flex', alignItems: 'center', gap: 10, background: WHITE }}>
                      <div style={{ fontSize: 11, color: TEXT_LIGHT, fontWeight: 600 }}>
                        Token <span style={{ color: TEXT_DARK, fontWeight: 800, fontSize: 12 }}>{tokenNo}</span>
                      </div>
                    ...
                    <div>
                      {kot.kot_items.map((ki, itemIdx) => {
                        ...
                        return (
                          <div key={ki.id}
                            style={{ padding: '10px 14px', borderBottom: itemIdx < kot.kot_items.length - 1 ? '1px solid ' + BORDER : 'none', display: 'flex', alignItems: 'center', gap: 10, background: isDone ? '#F0FDF4' : WHITE, transition: 'background 0.2s' }}>
                            {/* Qty */}
                            ...
                            {/* Toggle slider */}
                            <div onClick={() => toggleItem(ki.id, isDone)}
                    ...
                    <div style={{ padding: '12px 14px', borderTop: '1px solid ' + BORDER }}>
                      <button onClick={() => markFoodReady(kot)}

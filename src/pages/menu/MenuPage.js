...
const FOOD_TYPE = {
  veg:     { label: 'Veg',     color: '#15803D', bg: '#fee2e2', border: '#fecaca' },
  non_veg: { label: 'Non-Veg', color: '#B91C1C', bg: '#FEE2E2', border: '#FCA5A5' },
  egg:     { label: 'Egg',     color: '#b91c1c', bg: '#fee2e2', border: '#f87171' },
}
const PRIORITY_CONFIG = {
  1: { label: 'P1', color: '#B91C1C', bg: '#FEE2E2', border: '#FCA5A5', desc: 'Rush items, fire first' },
  2: { label: 'P2', color: '#b91c1c', bg: '#fee2e2', border: '#f87171', desc: 'Standard priority' },
  3: { label: 'P3', color: '#15803D', bg: '#fee2e2', border: '#fecaca', desc: 'Can wait, fire last' },
}
const UNITS = [
                  ...
                  <div key={item.id} style={{ background: isOn ? '#fff' : '#FAFAFA', borderRadius: 12, padding: '12px 16px', border: '1px solid ' + (isOn ? theme.border : '#FECACA'), display: 'flex', alignItems: 'center', gap: 12, opacity: isOn ? 1 : 0.75 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {itemAddonList.length > 0 && <span style={{ fontSize: 10, fontWeight: 700, background: '#fff5f5', color: '#b91c1c', border: '1px solid #fecaca', padding: '2px 7px', borderRadius: 10 }}>+ {itemAddonList.length} add-on{itemAddonList.length!==1?'s':''}</span>}
                      </div>
                      ...
                      {itemPortions.length > 0 && (
                        <div style={{ display: 'flex', gap: 4, marginTop: 4, flexWrap: 'wrap' }}>
                          {itemPortions.map(p => <span key={p.id} style={{ background: '#fff5f5', color: '#15803D', border: '1px solid #fecaca', fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 10 }}>{formatPortion(p)} · ₹{p.price}</span>)}
                        </div>
                      )}
                    </div>
                    ...
                    {isManager && (
                      <button onClick={() => toggleAvailability(item)} disabled={toggling === item.id}
                        style={{ background: isOn ? '#FEF2F2' : '#fee2e2', color: isOn ? '#B91C1C' : '#15803D', border: '1.5px solid ' + (isOn ? '#FECACA' : '#fecaca'), borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 800, cursor: 'pointer', flexShrink: 0, minWidth: 80, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <button onClick={() => setItemForm(f => ({ ...f, is_available: !f.is_available }))}
                    style={{ width: '100%', background: itemForm.is_available ? '#fee2e2' : '#FEE2E2', color: itemForm.is_available ? '#15803D' : '#B91C1C', border: '1.5px solid ' + (itemForm.is_available ? '#fecaca' : '#FECACA'), borderRadius: 9, padding: '10px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              ...
              <div style={{ borderTop: '2px solid ' + theme.bgWarm, paddingTop: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <button onClick={openAddAddon} style={{ background: '#b91c1c', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Add Add-on</button>
                </div>
                {addonList.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {addonList.map((a, idx) => (
                      <div key={idx} style={{ background: '#fff5f5', border: '1px solid #fecaca', borderRadius: 10, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: '#7f1d1d' }}>{a.name}</div>
                          <div style={{ fontSize: 11, color: '#b91c1c' }}>{a.price > 0 ? '+₹' + a.price : 'Free'}</div>
                        </div>
                ...
                {portionList.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {portionList.map((p, idx) => (
                      <div key={idx} style={{ background: '#fff5f5', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ flex: 1 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: 360, boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={saveAddon} disabled={savingAddon} style={{ flex: 2, background: '#b91c1c', color: '#fff', border: 'none', borderRadius: 8, padding: '11px', fontSize: 13, fontWeight: 700, cursor: savingAddon ? 'not-allowed' : 'pointer' }}>{savingAddon ? 'Saving...' : editAddonIdx !== null ? 'Save Add-on' : 'Add Add-on'}</button>
            </div>
          </div>
        </div>
      )}

...
const FOOD_TYPE = {
  ...
  non_veg: { label: 'Non-Veg', color: '#a93226', bg: '#fdedec', border: '#f1948a' },
  egg:     { label: 'Egg',     color: '#a93226', bg: '#fdedec', border: '#f1948a' },
}
const PRIORITY_CONFIG = {
  1: { label: 'P1', color: '#a93226', bg: '#fdedec', border: '#f1948a', desc: 'Rush items, fire first' },
  2: { label: 'P2', color: '#a93226', bg: '#fdedec', border: '#f1948a', desc: 'Standard priority' },
      ...
      <div style={{ width: 220, display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 14, border: '1px solid ' + theme.border, overflow: 'hidden' }}>
        <div style={{ padding: '14px 16px 10px', borderBottom: '1px solid ' + theme.border }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <button onClick={openAddCat} style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 6, width: 26, height: 26, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>+</button>
          </div>
          ...
          {categories.map((cat, idx) => {
            ...
            return (
              <div key={cat.id} draggable onDragStart={e => handleDragStart(e, idx)} onDragEnter={() => handleDragEnter(idx)} onDragOver={e => e.preventDefault()} onDragEnd={handleDragEnd} onClick={() => setActiveCategory(cat.id)}
                style={{ padding: '11px 12px', cursor: 'grab', background: isActive ? '#6b1f1f' : isDragOver ? '#fdf9f9' : 'transparent', borderBottom: '1px solid ' + (isDragOver ? '#c0392b' : theme.bgWarm), borderTop: isDragOver ? '2px solid #c0392b' : '2px solid transparent', opacity: isDragging ? 0.4 : 1, userSelect: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {cat.available_days?.length > 0 && <span style={{ fontSize: 9, background: isActive ? 'rgba(255,255,255,0.2)' : '#fdf9f9', color: isActive ? '#fff' : '#a93226', padding: '1px 4px', borderRadius: 4, fontWeight: 700, flexShrink: 0 }}>📅</span>}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            ...
            <p style={{ color: theme.textLight, fontSize: 13, marginTop: 3 }}>
              ...
              {disabledCount > 0 && <span style={{ marginLeft: 6, background: '#fdedec', color: '#a93226', fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 10 }}>{disabledCount} disabled</span>}
          ...
          <button onClick={openAddItem} disabled={!activeCategory} style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 9, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: activeCategory ? 'pointer' : 'not-allowed', opacity: activeCategory ? 1 : 0.5 }}>+ Add Item</button>
        </div>
          ...
          {activeItems.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {activeItems.map(item => {
                ...
                return (
                  <div key={item.id} style={{ background: isOn ? '#fff' : '#FAFAFA', borderRadius: 12, padding: '12px 16px', border: '1px solid ' + (isOn ? theme.border : '#fadbd8'), display: 'flex', alignItems: 'center', gap: 12, opacity: isOn ? 1 : 0.75 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {!isOn && <span style={{ fontSize: 10, fontWeight: 800, background: '#fdedec', color: '#a93226', padding: '2px 7px', borderRadius: 10 }}>OUT OF STOCK</span>}
                        {(item.available_from || item.available_until) && <span style={{ fontSize: 10, fontWeight: 700, background: '#fdf9f9', color: '#a93226', border: '1px solid #fadbd8', padding: '2px 7px', borderRadius: 10 }}>🕐 {item.available_from?.slice(0,5)||'00:00'} – {item.available_until?.slice(0,5)||'23:59'}</span>}
                        {itemVariations.length > 0 && <span style={{ fontSize: 10, fontWeight: 700, background: '#fdf9f9', color: '#922b21', border: '1px solid #f1948a', padding: '2px 7px', borderRadius: 10 }}>⚙ {itemVariations.length} variation{itemVariations.length!==1?'s':''}</span>}
                        {itemAddonList.length > 0 && <span style={{ fontSize: 10, fontWeight: 700, background: '#fdf9f9', color: '#C2410C', border: '1px solid #fadbd8', padding: '2px 7px', borderRadius: 10 }}>+ {itemAddonList.length} add-on{itemAddonList.length!==1?'s':''}</span>}
                      </div>
                    ...
                    {isManager && (
                      <button onClick={() => toggleAvailability(item)} disabled={toggling === item.id}
                        style={{ background: isOn ? '#FEF2F2' : '#DCFCE7', color: isOn ? '#a93226' : '#15803D', border: '1.5px solid ' + (isOn ? '#fadbd8' : '#86EFAC'), borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 800, cursor: 'pointer', flexShrink: 0, minWidth: 80, textAlign: 'center' }}>
                    ...
                    <button onClick={() => deleteItem(item.id)} style={{ background: theme.redBg, border: '1px solid #fadbd8', borderRadius: 7, padding: '6px 10px', fontSize: 12, fontWeight: 600, cursor: 'pointer', color: theme.red, flexShrink: 0 }}>🗑</button>
                  </div>
                )
              })}
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 32, width: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(day => {
                  ...
                  return (
                    ...
                    }} style={{ flex: 1, background: selected ? '#6b1f1f' : theme.bgWarm, color: selected ? '#fff' : theme.textMid, border: '1.5px solid ' + (selected ? '#6b1f1f' : theme.border), borderRadius: 8, padding: '8px 0', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                      {day}
                    </button>
                  )
                })}
              ...
              {catForm.available_days?.length > 0 && (
                <div style={{ marginTop: 8, background: '#fdf9f9', border: '1px solid #fadbd8', borderRadius: 8, padding: '7px 12px', fontSize: 11, color: '#a93226', fontWeight: 600 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={saveCat} disabled={saving} style={{ flex: 2, background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 9, padding: '12px', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>{saving ? 'Saving...' : editCat ? 'Save Changes' : 'Add Category'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 32, width: 560, maxHeight: '94vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <button onClick={() => setItemForm(f => ({ ...f, is_available: !f.is_available }))}
                    style={{ width: '100%', background: itemForm.is_available ? '#DCFCE7' : '#fdedec', color: itemForm.is_available ? '#15803D' : '#a93226', border: '1.5px solid ' + (itemForm.is_available ? '#86EFAC' : '#fadbd8'), borderRadius: 9, padding: '10px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {[0,5,12,18].map(rate => (
                    <button key={rate} onClick={() => setItemForm(f => ({ ...f, gst_rate: rate }))}
                      style={{ flex: 1, background: itemForm.gst_rate === rate ? '#6b1f1f' : theme.bgWarm, color: itemForm.gst_rate === rate ? '#fff' : theme.textMid, border: '1.5px solid ' + (itemForm.gst_rate === rate ? '#6b1f1f' : theme.border), borderRadius: 8, padding: '8px 0', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      {rate}%
                    </button>
                  ))}
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <input type="time" value={itemForm.available_from} onChange={e => setItemForm(f => ({ ...f, available_from: e.target.value }))}
                      style={{ width: '100%', border: '1.5px solid ' + theme.border, borderRadius: 9, padding: '9px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box', color: theme.textDark, background: itemForm.available_from ? '#fdf9f9' : '#fff' }} />
                  </div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <input type="time" value={itemForm.available_until} onChange={e => setItemForm(f => ({ ...f, available_until: e.target.value }))}
                      style={{ width: '100%', border: '1.5px solid ' + theme.border, borderRadius: 9, padding: '9px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box', color: theme.textDark, background: itemForm.available_until ? '#fdf9f9' : '#fff' }} />
                  </div>
              ...
              <div style={{ borderTop: '2px solid ' + theme.bgWarm, paddingTop: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <button onClick={openAddVariation} style={{ background: '#922b21', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Add Variation</button>
                </div>
                {variationList.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {variationList.map((v, idx) => (
                      <div key={idx} style={{ background: '#fdf9f9', border: '1px solid #f1948a', borderRadius: 10, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: '#6b1f1f' }}>{v.name}</div>
                          <div style={{ fontSize: 11, color: '#922b21' }}>₹{v.price}</div>
                        </div>
              ...
              <div style={{ borderTop: '2px solid ' + theme.bgWarm, paddingTop: 16 }}>
                ...
                {addonList.length === 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {addonList.map((a, idx) => (
                      <div key={idx} style={{ background: '#fdf9f9', border: '1px solid #fadbd8', borderRadius: 10, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div>
              ...
              <div style={{ borderTop: '2px solid ' + theme.bgWarm, paddingTop: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <button onClick={openAddPortion} style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ Add Portion</button>
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {portionList.map((p, idx) => (
                      <div key={idx} style={{ background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <button onClick={() => deletePortion(p, idx)} style={{ background: '#fdedec', border: '1px solid #fadbd8', borderRadius: 6, padding: '4px 8px', fontSize: 11, cursor: 'pointer', color: '#a93226' }}>🗑</button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              ...
              <button onClick={saveItem} disabled={saving} style={{ flex: 2, background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 9, padding: '12px', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>{saving ? 'Saving...' : editItem ? 'Save Changes' : 'Add Item'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: 360, boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={saveVariation} disabled={savingVariation} style={{ flex: 2, background: '#922b21', color: '#fff', border: 'none', borderRadius: 8, padding: '11px', fontSize: 13, fontWeight: 700, cursor: savingVariation ? 'not-allowed' : 'pointer' }}>{savingVariation ? 'Saving...' : editVariationIdx !== null ? 'Save Variation' : 'Add Variation'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: 380, boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={savePortion} disabled={savingPortion} style={{ flex: 2, background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 8, padding: '11px', fontSize: 13, fontWeight: 700, cursor: savingPortion ? 'not-allowed' : 'pointer' }}>{savingPortion ? 'Saving...' : editPortionIdx !== null ? 'Save Portion' : 'Add Portion'}</button>
            </div>
          </div>
        </div>
      )}

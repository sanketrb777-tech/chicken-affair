...
const ORDER_TYPES = {
  dine_in:  { label: 'Dine In',  icon: '🛋️', color: '#c0392b' },
  takeaway: { label: 'Takeaway', icon: '🛍️', color: '#cd6155' },
  delivery: { label: 'Delivery', icon: '🚚', color: '#c0392b' },
}
...
export function NewOrderPage() {
  ...
  async function fireKOT(hold, shouldPrint = false) {
    ...
    try {
      ...
      if (shouldPrint && kot) printKOT(kot, cart)
      setCart([])
      await fetchExistingOrder()
    } catch (err) {
  ...
  function printKOT(kot, cartItems) {
    const now = new Date()
    const dateStr = now.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: '2-digit' })
    const timeStr = now.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: false })
    const kotNo   = kot.id ? kot.id.replace(/-/g,'').slice(-5).toUpperCase() : '00001'
    const tableLabel = tableId ? 'Table ' + tableNumber : (existingOrder?.customer_name || orderType?.replace('_',' ') || 'Takeaway')
    ...
    ${cartItems.map(c => `
      <div class="item">
        ...
        <span class="name">${c.item.name}${c.variationName ? ' (' + c.variationName + ')' : ''}${c.portionName ? ' [' + c.portionName + ']' : ''}</span>
      </div>
      ${c.addons && c.addons.length > 0 ? `<div class="note">+ ${c.addons.map(a => a.name).join(', ')}</div>` : ''}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ flex: 1 }}>
            ...
            <input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="e.g. Rahul Sharma"
              style={{ width: '100%', border: '1.5px solid ' + (customerName ? '#c0392b' : theme.border), borderRadius: 8, padding: '9px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box', color: theme.textDark }} />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder="9876543210" type="tel"
              style={{ width: '100%', border: '1.5px solid ' + (customerPhone ? '#c0392b' : theme.border), borderRadius: 8, padding: '9px 12px', fontSize: 13, outline: 'none', boxSizing: 'border-box', color: theme.textDark }} />
          </div>
        </div>
        {orderType === 'delivery' && (
          <div>
  ...
  const menuPanel = (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
      ...
      {!isOffTable && (
        <div style={{ background: '#6b1f1f', borderRadius: 12, padding: '12px 16px', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ color: '#fff', fontWeight: 800, fontSize: 15 }}>
              {tableId ? 'Table ' + tableNumber : 'New Order'}
              {existingOrder && <span style={{ fontSize: 11, color: '#cd6155', marginLeft: 8, fontWeight: 600 }}>● Active</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>Covers:</span>
              <button onClick={() => setCovers(c => Math.max(1, c - 1))} style={{ background: '#6b1f1f', border: 'none', color: '#fff', width: 26, height: 26, borderRadius: 6, cursor: 'pointer', fontSize: 14 }}>-</button>
              <span style={{ color: '#fff', fontWeight: 700, minWidth: 18, textAlign: 'center', fontSize: 13 }}>{covers}</span>
              <button onClick={() => setCovers(c => c + 1)} style={{ background: '#6b1f1f', border: 'none', color: '#fff', width: 26, height: 26, borderRadius: 6, cursor: 'pointer', fontSize: 14 }}>+</button>
            </div>
          </div>
      ...
      {isOffTable && (
        <div style={{ background: '#6b1f1f', borderRadius: 12, padding: '10px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, overflowX: 'auto', paddingBottom: 4, flexShrink: 0 }}>
          {categories.map(cat => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
              style={{ background: activeCategory === cat.id ? '#6b1f1f' : '#fff', color: activeCategory === cat.id ? '#fff' : theme.textMid, border: '1px solid ' + theme.border, borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {cat.name}
            </button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8 }}>
          {displayItems.map(item => {
            ...
            return (
              <div key={item.id} style={{ background: '#fff', borderRadius: 10, padding: 12, border: qty > 0 ? '2px solid #c0392b' : '1px solid ' + theme.border, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 6 }}>
                  {itemVars.length > 0 ? (
                    ...
                    <div style={{ width: 9, height: 9, borderRadius: 2, border: '2px solid ' + (item.food_type === 'veg' ? '#15803D' : '#a93226'), background: item.food_type === 'veg' ? '#15803D' : '#a93226', flexShrink: 0, marginTop: 3 }} />
                  )}
                  <div style={{ fontWeight: 700, fontSize: 12, color: theme.textDark, lineHeight: 1.3 }}>{item.name}</div>
                </div>
                {hasOptions ? (
                  <div style={{ fontSize: 10, color: '#922b21', fontWeight: 700, marginBottom: 6 }}>
                ...
                <button onClick={() => addToCart(item)}
                  style={{ width: '100%', background: hasOptions ? '#922b21' : '#6b1f1f', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 0', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
  ...
  const orderPanel = (
    <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', flex: 1 }}>
      <div style={{ background: '#6b1f1f', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '12px 12px 0 0' }}>
        <div style={{ color: '#fff', fontWeight: 800, fontSize: 14 }}>Order Summary</div>
        {runningTotal > 0 && <div style={{ color: '#cd6155', fontWeight: 800, fontSize: 13 }}>₹{runningTotal.toFixed(0)} total</div>}
      </div>
          ...
          {existingKOTs.map((kot, idx) => (
            <div key={kot.id} style={{ padding: '10px 14px', borderBottom: '1px solid ' + theme.bgWarm }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: theme.textLight, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 8, background: kot.status === 'ready' ? '#DCFCE7' : '#fdedec', color: kot.status === 'ready' ? '#15803D' : '#a93226', padding: '1px 6px', borderRadius: 10, fontSize: 9 }}>{kot.status}</span>
              </div>
              {kot.kot_items.map((ki, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: theme.textMid, padding: '3px 0' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {ki.order_items?.variation_name && <span style={{ color: '#922b21', fontWeight: 600 }}> ({ki.order_items.variation_name})</span>}
                    {ki.order_items?.portion_name && <span style={{ color: '#c0392b', fontWeight: 600 }}> [{ki.order_items.portion_name}]</span>}
                  </div>
                  {editingKOTItem?.ki?.id === ki.id ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <input type="number" min="0" value={editQty} onChange={e => setEditQty(e.target.value)} autoFocus
                        style={{ width: 40, border: '1.5px solid #c0392b', borderRadius: 5, padding: '2px 5px', fontSize: 12, fontWeight: 700, textAlign: 'center', outline: 'none' }} />
                      <button onClick={saveKOTItemQty} style={{ background: '#c0392b', border: 'none', color: '#fff', borderRadius: 5, padding: '2px 7px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>✓</button>
                      ...
                      {isManager && (
                        <>
                          <button onClick={() => { setEditingKOTItem({ ki }); setEditQty(ki.order_items?.quantity || 1) }}
                            style={{ background: '#fdf9f9', border: 'none', borderRadius: 5, padding: '2px 6px', fontSize: 10, cursor: 'pointer', color: '#a93226', fontWeight: 700 }}>✏️</button>
                          <button onClick={() => deleteKOTItem(ki.id, ki.order_item_id)}
                            style={{ background: '#fdedec', border: 'none', borderRadius: 5, padding: '2px 6px', fontSize: 10, cursor: 'pointer', color: '#a93226', fontWeight: 700 }}>✕</button>
              ...
              <div key={c.key} style={{ padding: '8px 0', borderBottom: '1px solid ' + theme.bgWarm }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {c.variationName && <div style={{ fontSize: 11, color: '#922b21', fontWeight: 600 }}>{c.variationName}</div>}
                    {c.portionName && <div style={{ fontSize: 11, color: '#c0392b', fontWeight: 600 }}>{c.portionName}</div>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <button onClick={() => addToCart(c.item)} style={{ background: '#c0392b', border: 'none', borderRadius: 5, width: 22, height: 22, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#fff' }}>+</button>
                  </div>
                ...
                <input value={c.notes} onChange={e => setCart(prev => prev.map(ci => ci.key === c.key ? { ...ci, notes: e.target.value } : ci))}
                  ...
                  style={{ width: '100%', marginTop: 6, background: '#fdf9f9', border: '1px solid ' + theme.border, borderRadius: 6, padding: '5px 10px', fontSize: 11, color: theme.textDark, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}
          </>
        )}
      ...
      <div style={{ padding: '12px 14px', borderTop: '2px solid ' + theme.border, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <button onClick={() => fireKOT(true)} disabled={cart.length === 0 || submitting}
            style={{ background: cart.length === 0 ? theme.bgWarm : '#fdedec', color: cart.length === 0 ? theme.textMuted : '#a93226', border: '1px solid ' + (cart.length === 0 ? theme.border : '#f1948a'), borderRadius: 8, padding: '10px 0', fontSize: 12, fontWeight: 700, cursor: cart.length === 0 ? 'not-allowed' : 'pointer' }}>
            ⏸ Hold
          </button>
          <button onClick={() => fireKOT(false)} disabled={cart.length === 0 || submitting}
            style={{ background: cart.length === 0 ? theme.bgWarm : '#c0392b', color: cart.length === 0 ? theme.textMuted : '#fff', border: 'none', borderRadius: 8, padding: '10px 0', fontSize: 12, fontWeight: 700, cursor: cart.length === 0 ? 'not-allowed' : 'pointer' }}>
            {submitting ? 'Sending...' : '🔥 KOT'}
          </button>
        </div>
        {existingOrder && (
          <button onClick={() => navigate('/billing/order/' + existingOrder.id)}
            style={{ background: '#cd6155', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 0', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            ₹ Generate Bill
          </button>
        )}
      </div>
  ...
  return (
    <>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 20 }}
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: 28, width: '100%', maxWidth: 440, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              ...
              <div style={{ fontWeight: 900, fontSize: 16, color: '#6b1f1f' }}>₹{pickerItem.price}</div>
            </div>
            {(variations[pickerItem.id] || []).length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: theme.textDark, marginBottom: 12 }}>
                  Variation <span style={{ color: '#a93226', fontSize: 11 }}>* Required</span>
                </div>
                  ...
                  {(variations[pickerItem.id] || []).map(v => {
                    ...
                    return (
                      <button key={v.id} onClick={() => setPickerVariation(v)}
                        style={{ background: selected ? '#922b21' : '#fdf9f9', color: selected ? '#fff' : '#6b1f1f', border: '2px solid ' + (selected ? '#922b21' : '#f1948a'), borderRadius: 12, padding: '14px 10px', cursor: 'pointer', textAlign: 'center' }}>
            ...
            {(portions[pickerItem.id] || []).length > 0 && (variations[pickerItem.id] || []).length === 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: theme.textDark, marginBottom: 12 }}>
                  Portion / Size <span style={{ color: '#a93226', fontSize: 11 }}>* Required</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(portions[pickerItem.id] || []).map(p => (
                    <button key={p.id} onClick={() => confirmPicker(p)}
                      ...
                      <div style={{ fontWeight: 900, fontSize: 16, color: '#6b1f1f' }}>₹{p.price}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(addonGroups[pickerItem.id] || []).map(addon => {
                    ...
                    return (
                      <div key={addon.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: qty > 0 ? '#fdf9f9' : '#F9FAFB', borderRadius: 10, border: '1px solid ' + (qty > 0 ? '#fadbd8' : theme.border) }}>
                        <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          {qty > 0 && (
                            <>
                              <button onClick={() => adjustAddon(addon, -1)} style={{ background: '#fdedec', border: 'none', borderRadius: 6, width: 28, height: 28, fontSize: 16, cursor: 'pointer', fontWeight: 700, color: '#a93226', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>-</button>
                              <span style={{ fontWeight: 800, fontSize: 14, minWidth: 20, textAlign: 'center' }}>{qty}</span>
                            </>
                          )}
                          <button onClick={() => adjustAddon(addon, 1)}
                            style={{ background: '#6b1f1f', border: 'none', borderRadius: 6, width: 28, height: 28, fontSize: 16, cursor: 'pointer', fontWeight: 700, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                        </div>
                      </div>
                    )
                  })}
            ...
            {((variations[pickerItem.id] || []).length > 0 || (addonGroups[pickerItem.id] || []).length > 0) && (
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={() => confirmPicker(null)} disabled={!canConfirmPicker()}
                  style={{ flex: 2, background: canConfirmPicker() ? '#6b1f1f' : theme.bgWarm, color: canConfirmPicker() ? '#fff' : theme.textMuted, border: 'none', borderRadius: 10, padding: '12px', fontSize: 13, fontWeight: 700, cursor: canConfirmPicker() ? 'pointer' : 'not-allowed' }}>
                  Add to Order →
                </button>
              </div>
            )}
      ...
      {transferKOTItem && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2100, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 28, width: '100%', maxWidth: 380, boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            <h3 style={{ fontSize: 17, fontWeight: 800, color: '#6b1f1f', margin: '0 0 6px' }}>Transfer Item</h3>
            ...
            {activeTables.length === 0 ? (
              <div style={{ background: '#fdedec', borderRadius: 10, padding: '12px 14px', fontSize: 13, color: '#a93226', marginBottom: 20 }}>
                  ...
                  {activeTables.map(t => {
                    ...
                    return (
                      <button key={t.id} onClick={() => setTransferTargetId(t.id)}
                        style={{ width: 44, height: 44, background: isSelected ? '#6b1f1f' : isOccupied ? '#fdedec' : '#F3F4F6', color: isSelected ? '#fff' : isOccupied ? '#92400E' : '#374151', border: '2px solid ' + (isSelected ? '#6b1f1f' : isOccupied ? '#f1948a' : '#E5E7EB'), borderRadius: 8, fontSize: 14, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                        {t.number}
                      </button>
                    )
                  })}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={doTransfer} disabled={transferring || !transferTargetId || activeTables.length === 0}
                style={{ flex: 2, background: transferring || !transferTargetId ? '#E5E7EB' : '#6b1f1f', color: transferring || !transferTargetId ? '#9CA3AF' : '#fff', border: 'none', borderRadius: 10, padding: '12px', fontSize: 13, fontWeight: 700, cursor: transferring || !transferTargetId ? 'not-allowed' : 'pointer' }}>
      ...
      <div className="orders-mobile" style={{ display: 'none', flexDirection: 'column', height: 'calc(100vh - 120px)' }}>
        <div style={{ display: 'flex', background: '#6b1f1f', borderRadius: 10, padding: 4, marginBottom: 12, flexShrink: 0 }}>
          <button onClick={() => setMobileTab('menu')} style={{ flex: 1, padding: '8px 0', borderRadius: 7, border: 'none', fontWeight: 700, fontSize: 13, cursor: 'pointer', background: mobileTab === 'menu' ? '#fff' : 'transparent', color: mobileTab === 'menu' ? '#6b1f1f' : 'rgba(255,255,255,0.6)' }}>🍽 Menu</button>
          <button onClick={() => setMobileTab('order')} style={{ flex: 1, padding: '8px 0', borderRadius: 7, border: 'none', fontWeight: 700, fontSize: 13, cursor: 'pointer', background: mobileTab === 'order' ? '#fff' : 'transparent', color: mobileTab === 'order' ? '#6b1f1f' : 'rgba(255,255,255,0.6)' }}>
            🧾 Order {cartCount > 0 && <span style={{ background: '#c0392b', color: '#fff', borderRadius: 10, fontSize: 10, padding: '1px 6px', marginLeft: 4 }}>{cartCount}</span>}
          </button>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        ...
        <div ref={dropdownRef} style={{ position: 'relative' }}>
          <button onClick={() => setShowTypeMenu(d => !d)}
            style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 9, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div key={order.id} style={{ position: 'relative' }}>
                ...
                {canDelete && (
                  <button onClick={e => { e.stopPropagation(); setDeleteOrderId(order.id) }}
                    style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: '#FEF2F2', border: '1px solid #fadbd8', borderRadius: 7, padding: '6px 8px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
      ...
      {deleteOrderId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 360, textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#FEF2F2', border: '2px solid #fadbd8', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => deleteOrder(deleteOrderId)} style={{ flex: 1, background: '#c0392b', color: '#fff', border: 'none', borderRadius: 9, padding: '11px 0', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Delete</button>
            </div>
          </div>
        </div>
      )}

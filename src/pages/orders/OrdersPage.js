...
export function NewOrderPage() {
  ...
  const existingCustomerBadge = isOffTable && existingOrder && (
    <div style={{ background: ORDER_TYPES[orderType]?.color + '18', border: '1px solid ' + ORDER_TYPES[orderType]?.color + '33', borderRadius: 10, padding: '10px 14px', marginBottom: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
      ...
      <span style={{ marginLeft: 'auto', background: '#fee2e2', color: '#15803D', fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 20 }}>● Active</span>
    </div>
  )
          ...
          {existingKOTs.map((kot, idx) => (
            <div key={kot.id} style={{ padding: '10px 14px', borderBottom: '1px solid ' + theme.bgWarm }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: theme.textLight, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 8, background: kot.status === 'ready' ? '#fee2e2' : '#fee2e2', color: kot.status === 'ready' ? '#15803D' : '#b91c1c', padding: '1px 6px', borderRadius: 10, fontSize: 9 }}>{kot.status}</span>
              </div>
                      ...
                      {isManager && (
                        <>
                          ...
                          <button onClick={() => openTransfer(ki)}
                            style={{ background: '#fff5f5', border: 'none', borderRadius: 5, padding: '2px 6px', fontSize: 10, cursor: 'pointer', color: '#15803D', fontWeight: 700 }}>🔀</button>
                        </>
                      )}
                    </>
                  )}
              ...
              {kot.status !== 'ready' ? (
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <button onClick={() => markKOTReady(kot.id)} style={{ flex: 1, background: '#fee2e2', color: '#15803D', border: '1px solid #fecaca', borderRadius: 7, padding: '7px 0', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>✓ Mark as Ready</button>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <div style={{ flex: 1, background: '#fee2e2', color: '#15803D', border: '1px solid #fecaca', borderRadius: 7, padding: '7px 0', fontSize: 11, fontWeight: 700, textAlign: 'center' }}>✓ Ready</div>
                  <button onClick={() => printKOT(kot, existingKOTs[idx].kot_items.map(ki => ({ item: { name: ki.order_items?.menu_items?.name || '' }, quantity: ki.order_items?.quantity || 1, variationName: ki.order_items?.variation_name || null, portionName: ki.order_items?.portion_name || null, addons: ki.order_items?.addons || [], notes: ki.order_items?.notes || '' })))} style={{ background: '#fff5f5', color: '#b91c1c', border: '1px solid #fecaca', borderRadius: 7, padding: '7px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>🖨️</button>
                </div>
              )}
            </div>
              ...
              <div key={c.key} style={{ padding: '8px 0', borderBottom: '1px solid ' + theme.bgWarm }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {c.addons && c.addons.length > 0 && <div style={{ fontSize: 11, color: '#b91c1c' }}>+ {c.addons.map(a => a.name).join(', ')}</div>}
      ...
      <div style={{ padding: '12px 14px', borderTop: '2px solid ' + theme.border, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <button onClick={() => fireKOT(true)} disabled={cart.length === 0 || submitting}
            style={{ background: cart.length === 0 ? theme.bgWarm : '#fee2e2', color: cart.length === 0 ? theme.textMuted : '#b91c1c', border: '1px solid ' + (cart.length === 0 ? theme.border : '#f87171'), borderRadius: 8, padding: '10px 0', fontSize: 12, fontWeight: 700, cursor: cart.length === 0 ? 'not-allowed' : 'pointer' }}>
            ⏸ Hold
          </button>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {(portions[pickerItem.id] || []).map(p => (
                    <button key={p.id} onClick={() => confirmPicker(p)}
                      style={{ background: '#fff5f5', border: '2px solid #fecaca', borderRadius: 12, padding: '12px 16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                    ...
                    return (
                      <div key={addon.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: qty > 0 ? '#fff5f5' : '#F9FAFB', borderRadius: 10, border: '1px solid ' + (qty > 0 ? '#fecaca' : theme.border) }}>
                        <div>
                          ...
                          {addon.price > 0 && <div style={{ fontSize: 12, color: '#b91c1c', fontWeight: 600 }}>+₹{addon.price}</div>}
                        </div>
                  ...
                  {activeTables.map(t => {
                    ...
                    return (
                      <button key={t.id} onClick={() => setTransferTargetId(t.id)}
                        style={{ width: 44, height: 44, background: isSelected ? '#7f1d1d' : isOccupied ? '#fee2e2' : '#F3F4F6', color: isSelected ? '#fff' : isOccupied ? '#991b1b' : '#374151', border: '2px solid ' + (isSelected ? '#7f1d1d' : isOccupied ? '#f87171' : '#E5E7EB'), borderRadius: 8, fontSize: 14, fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                        {t.number}
                      </button>
                    )
                  })}
                ...
                <div
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {!isActive && <span style={{ fontSize: 10, fontWeight: 700, background: '#fee2e2', color: '#15803D', padding: '2px 8px', borderRadius: 20 }}>✓ Done</span>}
                    </div>

...
const CHART_COLORS = ['#6b1f1f', '#c0392b', '#cd6155', '#c0392b', '#a93226']

const ORDER_TYPE_BADGE = {
  dine_in:  { label: 'Dine In',  bg: '#fdedec', color: '#c0392b' },
  takeaway: { label: 'Takeaway', bg: '#fdedec', color: '#a93226' },
  delivery: { label: 'Delivery', bg: '#fdf9f9', color: '#c0392b' },
}
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 420, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ background: '#6b1f1f', borderRadius: '16px 16px 0 0', padding: '18px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button onClick={onClose} style={{ width: '100%', background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
            Close
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 14 }}>
            <div style={{ background: '#6b1f1f', borderRadius: 14, padding: '20px 22px' }}>
              ...
              <div style={{ fontSize: 28, fontWeight: 900, color: '#cd6155', letterSpacing: -1 }}>₹{totalRevenue.toFixed(0)}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            <div style={{ background: '#fff', borderRadius: 14, padding: '20px 22px', border: '1px solid ' + theme.border }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  { label: 'Card', value: cardTotal,  color: '#a93226' },
                  { label: 'UPI',  value: upiTotal,   color: '#a93226' },
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ background: '#fff', borderRadius: 14, padding: '20px 22px', border: '1px solid ' + theme.border }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 900, color: activeOrders > 0 ? '#a93226' : theme.textDark }}>{activeOrders}</div>
              </div>
              <div style={{ background: '#fff', borderRadius: 14, padding: '20px 22px', border: '1px solid ' + theme.border, flex: 1 }}>
                ...
                <div style={{ height: 7, background: theme.bgWarm, borderRadius: 99, overflow: 'hidden', marginBottom: 12 }}>
                  <div style={{ height: '100%', width: occupancyPct + '%', background: occupancyPct > 70 ? theme.red : occupancyPct > 40 ? '#cd6155' : '#c0392b', borderRadius: 99 }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  {[
                    { label: 'Free',     value: freeTables,     color: '#c0392b', bg: '#fdedec' },
                    { label: 'Occupied', value: occupiedTables, color: '#a93226', bg: '#fdedec' },
                  ...
                  <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 6px)', background: '#fff', border: '1px solid ' + theme.border, borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 100, minWidth: 160, overflow: 'hidden' }}>
                    {Object.entries(CHART_TYPE_LABELS).map(([key, label]) => (
                      <div key={key} onClick={() => { setChartType(key); setShowChartDropdown(false) }}
                        style={{ padding: '11px 16px', fontSize: 13, fontWeight: chartType === key ? 700 : 500, color: chartType === key ? '#fff' : theme.textDark, background: chartType === key ? '#6b1f1f' : 'transparent', cursor: 'pointer' }}
              ...
              <ResponsiveContainer width="100%" height={280}>
                {chartType === 'bar' ? (
                  <BarChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    <Bar dataKey="Dine In"  fill="#6b1f1f" radius={[4,4,0,0]} />
                    <Bar dataKey="Takeaway" fill="#c0392b" radius={[4,4,0,0]} />
                    <Bar dataKey="Delivery" fill="#cd6155" radius={[4,4,0,0]} />
                  </BarChart>
                ) : chartType === 'line' ? (
                  <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip formatter={(value) => ['₹' + value.toFixed(0)]} contentStyle={{ borderRadius: 10, border: '1px solid ' + theme.border, fontSize: 12 }} />
                    <Legend wrapperStyle={{ fontSize: 12 }} />
                    <Line type="monotone" dataKey="Dine In"  stroke="#6b1f1f" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey="Takeaway" stroke="#c0392b" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey="Delivery" stroke="#cd6155" strokeWidth={2} dot={{ r: 4 }} />
                  </LineChart>
                ) : (
                  <PieChart>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button
                                ...
                                style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#fdf9f9', border: 'none', borderRadius: 7, padding: '6px 12px', fontSize: 12, fontWeight: 600, color: '#a93226', cursor: 'pointer' }}>
                                <Eye size={13} /> View
                              </button>
                              <button

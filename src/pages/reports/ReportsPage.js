...
const ALL_FILTERS = ['Today', 'Yesterday', 'Last 7 Days', 'Last 30 Days', 'Custom Range']
const MANAGER_FILTERS = ['Today', 'Yesterday', 'Last 7 Days']
...
export default function ReportsPage() {
  const { profile, reportsMaxDays } = useAuth()
  const isOwner   = profile?.role === 'owner'
  const maxDays   = reportsMaxDays()
  const FILTERS   = maxDays ? MANAGER_FILTERS : ALL_FILTERS
  ...
  const [activeTab, setActiveTab]       = useState('overview') // 'overview' | 'category'
  ...
  async function fetchData() {
    ...
    // Enforce manager cap
    if (maxDays) {
      const cap = new Date(); cap.setDate(cap.getDate() - maxDays)
    ...
    const { data: bills } = await supabase
      .from('bills').select('id, total, cash_amount, card_amount, upi_amount, created_at, order_id')
    ...
    if (orderIds.length > 0) {
      const { data: ordersData } = await supabase.from('orders').select('id, order_type').in('id', orderIds)
    ...
    const { data: orderItems } = await supabase
      .from('order_items')
      .select('quantity, unit_price, menu_items(name, category_id, menu_categories(name))')
    ...
    ;(orderItems || []).forEach(oi => {
      ...
      const catName = oi.menu_items?.menu_categories?.name || 'Uncategorised'
      if (!name) return
      if (!itemMap[name]) itemMap[name] = { name, qty: 0, revenue: 0 }
      itemMap[name].qty     += oi.quantity
      itemMap[name].revenue += oi.quantity * parseFloat(oi.unit_price || 0)

      if (!catMap[catName]) catMap[catName] = { name: catName, qty: 0, revenue: 0 }
      catMap[catName].qty     += oi.quantity
      catMap[catName].revenue += oi.quantity * parseFloat(oi.unit_price || 0)
    })
    setItemSales(Object.values(itemMap).sort((a, b) => b.revenue - a.revenue))
    setCategorySales(Object.values(catMap).sort((a, b) => b.revenue - a.revenue))
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: theme.textLight, fontSize: 14, marginTop: 4 }}>
            ...
            {maxDays && <span style={{ marginLeft: 8, background: '#FEF3C7', color: '#B45309', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20 }}>Last {maxDays} days only</span>}
          </p>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {/* Tab switcher — owner only gets category tab */}
          {isOwner && (
            <div style={{ display: 'flex', background: theme.bgWarm, border: '1px solid ' + theme.border, borderRadius: 10, overflow: 'hidden' }}>
              <button onClick={() => setActiveTab('overview')}
                style={{ padding: '8px 16px', fontSize: 12, fontWeight: 700, border: 'none', cursor: 'pointer', background: activeTab === 'overview' ? '#092b33' : 'transparent', color: activeTab === 'overview' ? '#fff' : theme.textMid }}>
                Overview
              </button>
              <button onClick={() => setActiveTab('category')}
                style={{ padding: '8px 16px', fontSize: 12, fontWeight: 700, border: 'none', cursor: 'pointer', background: activeTab === 'category' ? '#092b33' : 'transparent', color: activeTab === 'category' ? '#fff' : theme.textMid }}>
                By Category
              </button>
            </div>
          )}
      ...
      {loading ? (
        ...
        <>
          ...
          {/* ── CATEGORY TAB (owner only) ── */}
          {activeTab === 'category' && isOwner && (
            <>
              {/* Category chart */}
              {card(<>
                {sectionTitle('Revenue by Category', 'How much each menu category earned')}
                {categorySales.length === 0 ? (
                  <div style={{ textAlign: 'center', color: theme.textMuted, fontSize: 13, padding: '30px 0' }}>No data in this period</div>
                ) : (
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={categorySales} margin={{ top: 5, right: 10, left: 0, bottom: 30 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={theme.bgWarm} />
              ...
              {/* Category table */}
              {card(<>
                {sectionTitle('Category-wise Breakdown', 'Sorted by revenue')}
                {categorySales.length === 0 ? (
                  <div style={{ textAlign: 'center', color: theme.textMuted, fontSize: 13, padding: '30px 0' }}>No items sold in this period</div>
                ) : (
                  <div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px', padding: '8px 12px', background: theme.bgWarm, borderRadius: 8, marginBottom: 4 }}>
                      {['Category', 'Items Sold', 'Revenue', '% Share'].map(h => (
                        <div key={h} style={{ fontSize: 11, fontWeight: 700, color: theme.textLight, textTransform: 'uppercase', letterSpacing: 0.5, textAlign: h === 'Category' ? 'left' : 'right' }}>{h}</div>
                      ))}
                    </div>
                    {categorySales.map((cat, idx) => {
                      const pct = totalRevenue > 0 ? ((cat.revenue / totalRevenue) * 100).toFixed(1) : 0
                      const barW = totalRevenue > 0 ? (cat.revenue / totalRevenue) * 100 : 0
                      return (
                        <div key={cat.name}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px', padding: '11px 12px', borderBottom: '1px solid ' + theme.bgWarm, alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <span style={{ fontSize: 11, fontWeight: 700, color: theme.textMuted, width: 20 }}>#{idx + 1}</span>
                              <span style={{ fontSize: 13, fontWeight: 700, color: theme.textDark }}>{cat.name}</span>
                            </div>
                            <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 700, color: theme.textMid }}>{cat.qty}</div>
                            <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 700, color: theme.textDark }}>₹{cat.revenue.toFixed(0)}</div>
                            <div style={{ textAlign: 'right', fontSize: 13, color: theme.textLight }}>{pct}%</div>
                          </div>
                          {/* Progress bar */}
                          <div style={{ height: 3, background: theme.bgWarm, overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: barW + '%', background: '#D4A853' }} />
                          </div>
                        </div>
                      )
                    })}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px', padding: '12px 12px', background: theme.bgWarm, borderRadius: 8, marginTop: 4 }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: theme.textDark }}>Total</div>
                      <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 800, color: theme.textDark }}>{categorySales.reduce((s, c) => s + c.qty, 0)}</div>
                      <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 800, color: theme.textDark }}>₹{totalRevenue.toFixed(0)}</div>
                      <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 800, color: theme.textDark }}>100%</div>
                    </div>
                  </div>

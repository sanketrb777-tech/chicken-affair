...
const STATUS_CFG = {
  free:           { bg: '#fdf9f9', border: '#fadbd8', borderStyle: '2px dashed', color: '#374151' },
  occupied:       { bg: '#fdf9f9', border: '#f1948a', borderStyle: '2px solid',  color: '#92400E' },
  bill_requested: { bg: '#FEF2F2', border: '#f1948a', borderStyle: '2px solid',  color: '#922b21' },
  reserved:       { bg: '#fdf9f9', border: '#f1948a', borderStyle: '2px solid',  color: '#922b21' },
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: theme.textLight, fontSize: 14, marginTop: 4 }}>
            <span style={{ color: '#c0392b', fontWeight: 700 }}>{freeCount} free</span>
            {' · '}
            <span style={{ color: '#a93226', fontWeight: 700 }}>{occupiedCount} occupied</span>
            {' · '}
            {tables.length} total
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {[
              { label: 'Free',     bg: '#fdf9f9', border: '#fadbd8', color: '#374151' },
              { label: 'Occupied', bg: '#fdf9f9', border: '#f1948a', color: '#92400E' },
              { label: 'Bill',     bg: '#FEF2F2', border: '#f1948a', color: '#922b21' },
            ].map(s => (
          ...
          {isManager && (
            <button onClick={openAdd}
              style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 9, padding: '9px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>
              + Add Table
            </button>
          )}
        </div>
      ...
      {tables.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48, color: theme.textLight, background: '#fff', borderRadius: 14, border: '2px dashed ' + theme.border }}>
          ...
          {isManager && <button onClick={openAdd} style={{ background: '#6b1f1f', color: '#fff', border: 'none', borderRadius: 8, padding: '9px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', marginTop: 8 }}>+ Add your first table</button>}
        </div>
      )}
          ...
          <div style={{ background: '#fff', borderRadius: 18, padding: 32, width: 400, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            {editTable && (
              <button onClick={() => deleteTable(editTable.id)}
                style={{ width: '100%', marginTop: 16, background: '#FEF2F2', border: '1px solid #fadbd8', borderRadius: 9, padding: '10px', fontSize: 13, fontWeight: 700, cursor: 'pointer', color: '#c0392b' }}>
                🗑 Delete this table
              </button>
            )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
                {freeTables.map(t => (
                  <button key={t.id} onClick={() => handleChangeTable(t)} disabled={changingTable}
                    style={{ background: '#fdf9f9', border: '2px solid #fadbd8', borderRadius: 12, padding: '14px 10px', cursor: 'pointer', textAlign: 'center', minWidth: 80 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#c0392b', fontWeight: 700, marginTop: 4 }}>Free</div>
                  </button>
                ))}
              </div>
            )}

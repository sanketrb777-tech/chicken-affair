...
export default function InventoryPage() {
  ...
  return (
    <div>
      ...
      {/* Low stock banner */}
      {lowCount > 0 && (
        <div style={{ background: '#fff5f5', border: '1px solid #f87171', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => setFilter('low')}
            style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 700, color: '#dc2626', background: 'none', border: '1px solid #f87171', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>
            View Low Stock
          </button>
        </div>
      )}
              ...
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 120px 120px 160px', padding: '14px 18px', borderBottom: idx < filtered.length - 1 ? '1px solid ' + theme.bgWarm : 'none', alignItems: 'center', background: isLow ? '#fff5f5' : '#fff', transition: 'background 0.1s' }}>
                ...
                <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                  <button onClick={() => openAdjust(item)}
                    style={{ background: '#fff5f5', border: '1px solid #fee2e2', borderRadius: 7, padding: '6px 12px', fontSize: 12, fontWeight: 700, color: '#15803D', cursor: 'pointer' }}>
                    ± Stock
                  </button>
